...
- Deployed via **Cloudflare Workers & Pages** (`manas-farm-dashboard` project), auto-deploying from the `main` branch on GitHub (`songxran/manas-farm-dashboard`) — pushing to `main` is the deploy step, there is no separate build/publish command. Live at `https://manas-farm-dashboard.songkran9091.workers.dev/`. Was previously on Netlify (`manasfarm.netlify.app`); migrated after Netlify's free-tier "operational credits" quota was exhausted mid-session (Sept 2026) and production deploys got paused there.
  - `wrangler.toml` (`[assets] directory = "."`) exists solely so Cloudflare's Git-connected build — which defaults to a Workers-style `npx wrangler deploy` command rather than a plain static build — knows to treat the whole repo as a static asset bundle. There is no actual Worker/server code; don't add any without realizing this reconfigures the deploy target.
  - `.assetsignore` excludes `.git/`, `wrangler.toml`, and the `.md` docs from the uploaded static assets — without it, wrangler happily uploads `.git/objects` etc. as publicly-served files (this happened once and was fixed).
...
- **Cycle revenue**: `production_cycles.revenue` (JS `cycle.revenue`) is the *actual* baht received for that harvest, entered in the production edit form. The report uses it directly when set; when `null` it falls back to an estimate (`yieldKg × FARM_SETTINGS.sell_price`) and tags the row "(ประมาณ)". `renderReport` / the CSV export both branch on `c.revenue != null`.
- **Report rows**: `renderReport` iterates every pond in `PONDS` (not just those with cycles). A pond with no cycle gets a muted "ยังไม่เริ่มรุ่น" row showing its accumulated `costTotal`; a pond that has cycles but also cost items with `cycleId = null` gets an extra "ต้นทุนไม่ระบุรุ่น" row for the difference. Both carry cost (negative profit, no revenue) and ARE summed into the totals, so the report's ต้นทุนรวม always equals the cost page's grand total.
...
- Edit an existing cycle: `#productionModal`, `openProductionForm(cycleId)` — takes a **cycle id**, not a pond id. Edits `cycle_no` + `release_date` (this is the only place to correct those after creation — the pond form's "เริ่มรุ่นที่" section is add-only) plus yield/survival/fcr/harvest/grade/revenue. A `cycle_no` collision with another cycle of the same pond surfaces as a friendly message via the `23505` (unique-violation) branch in the save handler.
